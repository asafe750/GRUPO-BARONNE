...
import { PALLET_IMAGE, TRANSPORT_IMAGE, WHATSAPP } from "@/lib/constants";
    ...
    >
      <div className="mx-auto grid max-w-7xl gap-6 lg:grid-cols-2">
        <article className="card-hover relative isolate overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900 p-7 md:p-9">
          ...
          />
          <div className="absolute inset-0 -z-10 bg-[linear-gradient(135deg,rgba(0,0,0,0.88),rgba(0,0,0,0.74))]" />
        ...
        <article className="card-hover relative overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900 p-7 md:p-9">
          <Image
            src={PALLET_IMAGE}
            alt="Paletes de madeira em armazém para venda de paletes PBR certificados em Camaçari BA"
            fill
            sizes="(min-width: 1024px) 50vw, 100vw"
            className="absolute inset-0 object-cover opacity-28"
          />
          <div className="absolute inset-0 bg-[linear-gradient(135deg,rgba(0,0,0,0.86),rgba(0,0,0,0.68))]" />
          <div className="relative">
            <PalletIcon />
            <h2 className="mt-8 font-heading text-5xl font-black uppercase leading-none text-white">
            ...
            <p className="mt-6 rounded-2xl border border-brand-green/35 bg-brand-green/10 px-5 py-4 font-heading text-2xl font-black uppercase text-brand-accent">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              {["100% PBR", "Certificado", "Pronta Entrega"].map((tag) => (
                <span
                  key={tag}
                  className="rounded-full border border-brand-green/40 bg-brand-green/10 px-4 py-2 text-xs font-extrabold uppercase tracking-[0.16em] text-brand-accent"
                >
                  {tag}
                </span>
              ))}
